...
(** Briefly explain the difference between the tactics [apply] and
    ...
    applied?
its sugar for rewrite -> x. reflexivity.
*)
(** [] *)
...
Proof.
  ...
  apply trans_eq with (m:=minustwo o). rewrite <- H1.
...
Example sillyex2 : forall (X : Type) (x y z : X) (l j : list X),
     ...
     x = z.
Proof.
  intros X x y z l j eq eq2. inversion eq. Qed.
(** [] *)
...
Proof.
  intros n eq. destruct n. reflexivity. inversion eq. Qed.
...
Proof.
  ...
  Case "n = S n'". intros m H. destruct m as [| m'].
    ...
    SCase "m = S m'". rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H.
...
Proof.
  intros n. induction n as [| n'].
  Case "n = 0". intros m. destruct m as [| m']. reflexivity.
  SCase "m = S m'". intros eq. inversion eq.
  Case "n = S n'". intros m eq. destruct m as [| m'].
    SCase "m = 0". inversion eq.
    SCase "m = S m'". apply f_equal. rewrite <- IHn'. reflexivity. 
...
Proof.
  ...
  Case "l = l' :: l". intros n eq. destruct n as [| n'].
    SCase "n = 0". simpl. inversion eq.
    SCase "n = S n'". simpl. apply f_equal. apply IHl. inversion eq.
    reflexivity. Qed. 
(** [] *)
...
Theorem app_length_cons : forall (X : Type) (l1 l2 : list X) 
     ...
     S (length (l1 ++ l2)) = n.
Proof.
  intros X l1 l2 x n. generalize dependent n. induction l1 as [| l'].
  Case "l = []". simpl. intros n eq. inversion eq. reflexivity.
  Case "l = cons". simpl. intros n. destruct n as [| n'].
    SCase "n = 0". intros eq. inversion eq.
    SCase "n = S n'". intros eq. apply f_equal. apply IHl1. inversion eq.
    reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  Case "n = S n'". intros m. destruct m as [| m'].
    ...
    SCase "m = S n'". simpl. apply IHn'. Qed.
(** [] *)
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  intros n m p H H2. apply beq_nat_true in H. rewrite H. apply H2. Qed.
(** [] *)
